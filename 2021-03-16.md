...
* Do we need a fencing for a crashed node?
  * Yes - raft paper suggets a solution
...
* Raft allows overwriting entries in the log. PG WAL doesn't
  * Resolved: we treat raft log as ephmeral and treat only those entries before leader commitIndex as immutable. Technically leader log entries can be overwritten. So we need to wait until one entry to consider it to be really immutable.
...
* What is the relationship between python implementation of raft and the rust implementation?
  * [python implementation](https://github.com/adsharma/raft)
    ...
    * Written circa 2015 and seems to be unmaintined
  ...
  * Arguments for Python
    ...
    * Heidi Howard [wrote](https://twitter.com/heidiann360/status/1254701717150720000/photo/1) one for paxos too, but it has many details that an implementor has to figure out vs Raft
    * We are proposing some changes to Raft
    * Python could be seen as an in-between step between TLA+ and production Rust code.
    * Write tests, debug them easily and once satisified rewrite in rust
* What is out-of-order raft and why is it needed?
  ...
  * It does so by [nacking](https://github.com/ongardie/raft.tla/blob/master/raft.tla#L286) out or order responses. `logOk` will be false.
